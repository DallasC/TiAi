...
import datetime
from environment.utils import get_mysql_variables
...
def get_instance_current_knobs(instance):
    """
    get instance current knobs
    :param instance: instance name  example:mysql3
    :return: current_knobs
    """
    mysql_variables = get_mysql_variables(configs.instance_config[instance])
    current_knobs = {}
    for knob in KNOBS:
        current_knobs[knob] = int(mysql_variables.get(knob))

    print("get_instance_current_knobs: ", current_knobs)
    return current_knobs
...
def save_knobs(knob, metrics, knob_file, run_identifying=None):
    ...
    print("save_knobs.knob:", knob)
    print("save_knobs.metrics:", metrics)
    print("save_knobs.knob_file:", knob_file)
    ...
    result_str = '{}#{}#{}#'.format("tps:"+str(metrics[0]), "latency:"+str(metrics[1]), "qps:"+str(metrics[2]))
    knob_str = "#".join(knob_strs)
    result_str += knob_str
    result_str += "#run_identifying:'{}'".format(run_identifying if run_identifying else '')
    result_str += "#writed_at:'{}'".format(str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
