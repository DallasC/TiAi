...
class MySQLEnv(object):
    ...
    def __init__(self, wk_type='read', method='sysbench', num_other_knobs=0, num_metric=63, alpha=1.0, beta1=0.5, beta2=0.5,
                 time_decay1=1.0, time_decay2=1.0):
    ...
    def step(self, knob):
        ...
        knobs.save_knobs(
            ...
            run_identifying=self.run_identifying
        )
    ...
    def _get_state(self, knob, method='sysbench'):
        ...
        print("_get_state.knob:", knob)
        print("_get_state.method:", method)
    ...
    def get_current_knobs(instance):
        knob_values = []
        for name in knobs[instance]:
            knob_values[name] = utils.get_mysql_variables(configs[instance], name)
...
class Server(MySQLEnv):
    ...
    def __init__(self, wk_type, instance_name, run_identifying=None):
        ...
        self.run_identifying = run_identifying
    ...
    def initialize(self, instance_knobs=None, apply_knobs=True):

        """
        Initialize the environment when an episode starts
        :param knobs: knobs  if none will set default_knobs
        :param apply_knobs:  modify instance configurations
        :return: state: np.array, current state
        """
        ...
        if not instance_knobs:
            # config default values
            instance_knobs = self.default_knobs

        flag = True
        if apply_knobs:
            flag = self._apply_knobs(instance_knobs)
        ...
        external_metrics, internal_metrics = self._get_state(knob=instance_knobs, method=self.method)
    ...
    def _apply_knobs(self, knob):
        ...
        print("test_mysql:", flag)
        while not flag and steps < max_steps:
            time.sleep(5)
            flag = utils.test_mysql(self.instance_name)
            steps += 1
            print("flag: %s steps: %s max_steps: %s" % (flag, steps, max_steps))
